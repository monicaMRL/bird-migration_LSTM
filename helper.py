...
def crossvalidatetype2(birds,y1,y2,modelname,name,plot=False):
	...
	k=[5,10,15,20,25,30,35,40]
		...
		for i,bird in enumerate(birds):
   		...
   		error,stdev = evaluatetype2(X_train,y_long_train,y_lat_train,X_test,y_long_test,y_lat_test,modelname,name,plot=False)
	...
	plt.savefig(path,format="png")
	plt.clf()
        return[np.mean(cverror),np.mean(cvstdev)]  	
